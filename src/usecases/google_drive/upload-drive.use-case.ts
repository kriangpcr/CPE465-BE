...
> {
    ...
    async execute(ctx: { body: UploadDriveRequestDto, files: Express.Multer.File[] }): Promise<any> {
        const files = ctx.files;
        const date = ctx.body.date;
        const jobId = ctx.body.jobId;
        const subfolderName = ctx.body.subfolderName;

        return await this.googleDriveService.uploadFile(date, jobId, subfolderName, files);
    }
}
